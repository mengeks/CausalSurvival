...
for n in 200 500 1000 2000; do
  for eta_type in "linear" "non-linear"; do
    for HTE_type in "linear" "non-linear"; do
      ...
      $singularity_command Rscript -e "source('scripts/TV-CSL/TV-CSL-runner.R'); run_experiment_iteration(${SLURM_ARRAY_TASK_ID}, '$json_file', '$eta_type', '$HTE_type', $n, $verbose)" > "$log_dir/log_iteration_${SLURM_ARRAY_TASK_ID}.txt" 2>&1
